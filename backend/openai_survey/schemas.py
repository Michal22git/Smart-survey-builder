...
class QuestionSchema(BaseModel):
    ...
    def validate_options(cls, v, values):
        ...
        if values.get("type") in ["radio", "checkbox", "dropdown"] and (
            not v or len(v) < 2
        ):
            raise ValueError(
                f"Questions of type {values.get('type')} must have at least 2 options"
            )
        return v
...
class SurveySchema(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, description="Description of the survey"
    )
...
class SurveyGenerationRequest(BaseModel):
    ...
    template: Optional[str] = Field(
        default="general", description="Survey template type to use"
    )
    num_questions: Optional[int] = Field(
        default=5, description="Suggested number of questions"
    )
    language: Optional[str] = Field(default=None, description="Language for the survey")
...
class SurveyAnalysisRequest(BaseModel):
    ...
    max_responses: Optional[int] = Field(
        default=100, description="Maximum number of responses to analyze"
    )
